...
通过 + iOS 模拟器（`iphonesimulator`）跑完整套件全绿；真机验证（签名强制 + arm64e ptrauth）留作后续
（见下）。下一步向其它平台横向铺开。
...
行为套件实跑)。**真机验证(设备端签名强制 + arm64e ptrauth)留作后续**:需越狱设备自建 runner 或 Corellium
订阅——CI 矩阵里保留了一个 planned 占位项作为口子。其它 OS 还需各自的 backend。
