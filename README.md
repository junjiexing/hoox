...
（`gcc-arm-linux-gnueabihf`）后在 `qemu-arm` 下跑完整测试套件。下一步向其它平台横向铺开。
...
可直接使用的组合是 **Windows × (x86 / x86_64 / ARM64)** 与 **Linux × (x86 / x86_64 / ARM / ARM64)**。
...
ARM 走自研 A32+Thumb 解码器 `src/disasm/hx_disasm_arm.c` 驱动 `src/arch/arm`。其它 OS 还需各自的
backend。MIPS 在 frida 本身即为部分/实验性支持。
