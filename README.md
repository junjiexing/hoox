...
（`gcc-arm-linux-gnueabihf`）后在 `qemu-arm` 下跑完整测试套件。**macOS x86_64 亦已打通**（原生
`macos-15-intel` CI，AppleClang，含 interceptor 行为测试全绿）。下一步向其它平台横向铺开。
...
ARM 走自研 A32+Thumb 解码器 `src/disasm/hx_disasm_arm.c` 驱动 `src/arch/arm`。**macOS x86_64**
复用 x86 解码器/arch 与 POSIX 分配器 + pthread TLS，新增 `src/backend/darwin`（mach VM 查询/枚举、
dyld 模块枚举）；关键点是 patch 代码签名过的 `__TEXT` 页需用 `mach_vm_protect` 加 `VM_PROT_COPY`
（在 `macos-15-intel` 原生 CI 上验证）。其它 OS 还需各自的 backend。MIPS 在 frida 本身即为部分/实验性支持。
