...
† iOS ARM64：设备 SDK 交叉编译 + iOS 模拟器实跑全绿；**真机（越狱设备上的签名强制 + arm64e ptrauth）尚未测试**，后续本地验证。
...
行为套件实跑)。**⚠️ iOS 真机尚未测试**:设备端签名强制 + arm64e ptrauth 只有在真机上才能验证,需越狱设备
(后续本地验证,不在 CI 范围内)。其它 OS 还需各自的 backend。
