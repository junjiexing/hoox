...
struct _HooxX86FunctionContextData
{
  ...
  /* FAST near-redirect fallback (64-bit only; 32-bit always plans a near
   * redirect, and the backend_data budget has no room for this flag). */
                                            ...
                                            hx_boolean force)
{
  ...
  if (!force && !hoox_x86_relocator_can_relocate (ctx->function_address,
        data->redirect_code_size, scenario, NULL))
    {
       ...
       * replacement. (32-bit already plans a near redirect, so there is
