...
- **MSVC `/GF`（字符串池化）**：顶层 CMakeLists 已为 MSVC 开启，使相同字面量共享同一地址，
  匹配 gcc/clang 默认行为（有测试按指针相等比较字面量）。
...
Windows/Linux/非越狱走 RWX；Apple 平台单独实现 `hooxcodesegment-darwin`。详见 `docs/PLAN.md` §6。
