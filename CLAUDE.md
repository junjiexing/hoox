...
桩里 `mach_vm_protect(RW|COPY)`→内联 8-byte 复制循环→`mach_vm_protect(RX)`
（桩返回 `kern_return_t`，protect 被拒时干净失败不静默）；丢 X 窗口里执行的指令只在桩页（外加 shared cache
的 `mach_vm_protect`），与目标页布局**无关**故结构性正确；与 `mach_vm_protect` 同页的极端目标会在安装前拒绝。
内联循环写整页且桩大小固定，
...
**iOS ARM64 已接入（设备 SDK 交叉编译 + iOS 模拟器实跑,CI 全绿）：** iOS 复用 `src/backend/darwin` +
...
(`test_memory`/`interceptor_smoke` 自跳过)。**iOS 真机已验证：** iPhone 6s（A9/arm64，非 arm64e）/
iOS 15.8.2 / Dopamine 越狱上 interceptor 全套 + amalgam + selfhost 全绿；真机不在 CI 范围内，
arm64e ptrauth 路径现由 `__arm64e__`/`__has_feature(ptrauth_calls)` 自动启用；真机回归仍未覆盖。
...
下一步：其它平台（tvOS 真机与 iOS arm64e 由作者后续本地验证）。
