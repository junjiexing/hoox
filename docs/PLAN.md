...
`mach_vm_protect(RW|COPY)` → 内联 8-byte 复制循环 → `mach_vm_protect(RX)`；丢 X
窗口里执行的指令只在桩页（外加 shared cache 里的 `mach_vm_protect`），与目标页布局**无关**；唯一无法安全
处理的是目标与 `mach_vm_protect` 自身同页，此时安装前会拒绝。内联循环写回整页且桩大小固定、无变更字数
上限，同时天然覆盖多种情形：
