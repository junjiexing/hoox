...
2. **保留 Frida inline hook 的核心平台/架构支持**：OS（Windows / Linux / Android /
   macOS / iOS / tvOS / FreeBSD / QNX）× 架构（x86 / x86_64 / ARM / ARM64 / MIPS），
   **按 §6 矩阵分阶段覆盖**（覆盖度与 Frida 对齐——例如 **MIPS 与 Frida 同等，为部分/实验性支持**，
   非完整）。并**保留 frida 中 hook 相关的测试**用于验证。
...
| Interceptor 架构后端 | `gum/backend-x86/guminterceptor-x86.c`（arm/arm64/mips 同构） | 生成/激活 trampoline |
...
- 需要“重写”的指令是**有界集**：arm64≈8、thumb≈11、arm≈8、mips 实际 1、x86 分支族
...
每架构须过 frida relocator 测试方算通过。MIPS 定长 4 字节、~20 个分支 id，手写小解码器即可。
...
| **M9** | 横向架构铺开 | ARM64 → ARM/Thumb → MIPS backend+arch+测试；Android/FreeBSD/QNX backend（MIPS 与 frida 同等，见 §6 注） |
...
| OS \ Arch | x86 | x86_64 | ARM | ARM64 | MIPS | 备注 |
|---|:--:|:--:|:--:|:--:|:--:|---|
| Windows | ✓ | ✓(M6) | – | ✓ | – | RWX 页；codesegment=stub |
| Linux | ✓ | ✓(M8) | ✓ | ✓ | ⚠️ | posix tls/exceptor stub；MIPS 见下注 |
...
> ⚠️ **MIPS（`⚠️`）与 frida 同等（部分/实验性）**：已核实 frida 的 mips relocator 仅实现
> `MIPS_INS_B`，`J` 及所有条件分支均为 `g_assert_not_reached()`。hoox 不承诺超出 frida 的
> MIPS 覆盖；标 `✓` 的架构为完整支持。
...
| Detours 无 MIPS / ARM 语义位需补 | 中 | MIPS 定长手写小解码器；ARM/ARM64 语义位从定长编码位域解出（参考 Detours）；各以对应 relocator 测试验收 |
