...
on_enter (HooxInvocationContext * ic, hx_pointer user_data)
{
  ...
  printf ("  [listener] enter: compute(%d, %d)  thread=%u depth=%u\n",
      a, b, hoox_invocation_context_get_thread_id (ic),
...
main (void)
{
  ...
  ar = hoox_interceptor_attach (interceptor, (hx_pointer) compute, listener, NULL);
  ...
  rr = hoox_interceptor_replace (interceptor, (hx_pointer) compute,
      (hx_pointer) replacement_compute, (hx_pointer *) &original_compute, NULL);
  if (rr != HOOX_REPLACE_OK)
  {
